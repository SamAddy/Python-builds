...
words_dict['A'] = [input() for _ in range(n)]

words_dict['B'] = [input() for _ in range(m)]

indices = []

for letter in words_dict['B']:
    letter_indices = [index + 1 for index, value in enumerate(words_dict['A']) if value == letter]
    if letter_indices:
        indices.append(letter_indices)
    else:
        indices.append([-1])
