...
def closestNumbers(arr):
    ...
    # pairs = [(i, j) for i in range(len(arr)) for j in range()]
    arr.sort()
    ...
    for i in range(n - 1):
        current_value = arr[i]
        next_value = arr[i + 1]
        abs_diff = abs(current_value - next_value)

        if abs_diff < min_value:
            min_value = abs_diff
            combs = [current_value, next_value]

        elif abs_diff == min_value:
            combs.extend([current_value, next_value])
