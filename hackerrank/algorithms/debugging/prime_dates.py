...
As is divisible by 4 so the date "02-08-2024" is called a lucky date.
...
The only line of the input contains two strings u and v and denoting the two dates following the format dd-mm-yyyy.
Consider, d is the day number, m is the month number and y is the year number.

Note: Here m = 01 means January, m = 02 means February, m = 03 means March and so on and all the dates follow the
standard structure of English calender including the leap year.
...
def updateLeapYear(year):
    if year % 400 == 0:
        month[2] = 29 # In a leap year February has 29 days
    elif year % 100 == 0:
        month[2] = 28 # If year is divisble by 100 then it isnt a leap year hence February has 28 days
    elif year % 4 == 0:
        month[2] = 29
    else:
        month[2] = 28
...
def findPrimeDates(d1, m1, y1, d2, m2, y2):
    ...
    while(True):
        x = d1
        x = x * 100 + m1
        x = x * 10000 + y1 # Changed 1000 to 10000 to make better addition
        if x % 4 == 0 or x % 7 == 0: # Changed and to or since the instruction said 4 or 7
            result = result + 1
        ...
        if d1 > month[m1]:
            ...
            if m1 > 12:
                y1 =  y1 + 1
                m1 = 1 # Changed m1 = m1 + 1 to m1 = 1 -- In a new year we start from month 1
    return result
