"""
Neo has a complex matrix NxM script. The matrix script is a X
...
def decode_matrix(matrix_):
    transposed_matrix = list(map(''.join, zip(*matrix_)))
    decoded_string = ''.join(transposed_matrix)
    cleaned_decoded_string = re.sub(r'(?<=[A-Za-z0-9])[^A-Za-z0-9]+(?=[A-Za-z0-9])', ' ', decoded_string)
    return cleaned_decoded_string
