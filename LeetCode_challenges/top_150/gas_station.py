...
Solution approach:
- Considered that for there to be a solution, sum(cost) must be greater than or equal to the sum(gas) and so
if sum(cost) < sum(gas), then we return -1 knowiing that we cant travel full circuit

- We then iterate through the gas array calculating the difference between gas[i] and cost[i] and then keep track of the
index in which we obtain a positive tank value
"""
